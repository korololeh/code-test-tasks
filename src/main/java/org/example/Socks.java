...
public class Socks {
        ...
        public static int sockMerchant(int n, List<Integer> ar) {
            ...
            Map<Integer, Long> socks = ar.stream().collect(Collectors.groupingBy(Integer::intValue, Collectors.counting()));
            return socks.values().stream().mapToInt(Long::intValue).filter(s->s>1).map(x-> (x - (x % 2))/2).reduce(0, Integer::sum);
        }
